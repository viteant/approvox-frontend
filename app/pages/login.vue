<template>
  <div class="min-h-screen flex items-center justify-center bg-azure-white dark:bg-deep-navy transition-colors duration-500 overflow-hidden relative">
    ...
    >
      <!-- DEBUG INFO -->
      <pre v-if="user" class="hidden">{{ JSON.stringify(user, null, 2) }}</pre>
      ...
      >
        <div class="text-center mb-10">
          <div class="inline-flex p-4 rounded-3xl bg-blue-50 dark:bg-blue-900/30 mb-6 shadow-sm">
            <UIcon name="i-lucide-shield-check" class="w-10 h-10 text-primary-600 dark:text-primary-400" />
          </div>
        ...
        <UForm :state="state" class="space-y-6" @submit="handleSubmit">
          <UFormField :label="$t('auth.email')" name="email" required>
            <UInput
              v-model="state.email"
              type="email"
              icon="i-lucide-mail"
              placeholder="example@approvox.com"
              class="w-full"
            />
          </UFormField>
